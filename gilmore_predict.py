...
def predict_gilmore_week():
    df = pd.read_csv('static/gilmore.csv', parse_dates=['date'])
    time_slots = [
    ...
    if len(day_data) >= 4:
        ...
        plt.title(f'Gilmore Station Predicted Entries & Exits per Hour ({today_short})')
        plt.legend()
        plt.tight_layout()
        plt.savefig('static/images/gilmore_predicted_bar.png')
        plt.close()
