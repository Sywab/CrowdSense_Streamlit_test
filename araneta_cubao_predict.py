...
def predict_araneta_cubao_week():
    df = pd.read_csv('static/araneta.cubao.csv', parse_dates=['date'])
    time_slots = [
    ...
    if len(day_data) >= 4:
        ...
        plt.title(f'Araneta Center Cubao Terminal Station Predicted Entries & Exits per Hour ({today_short})')
        plt.legend()
        plt.tight_layout()
        plt.savefig('static/images/araneta_cubao_terminal_predicted_bar.png')
        plt.close()
