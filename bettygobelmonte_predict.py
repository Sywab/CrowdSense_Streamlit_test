...
def predict_bettygobelmonte_week():
    df = pd.read_csv('static/betty.go.belmonte.csv', parse_dates=['date'])
    time_slots = [
    ...
    if len(day_data) >= 4:
        ...
        plt.title(f'Betty Go-Belmonte Station Predicted Entries & Exits per Hour ({today_short})')
        plt.legend()
        plt.tight_layout()
        plt.savefig('static/images/betty_go_belmonte_predicted_bar.png')
        plt.close()
