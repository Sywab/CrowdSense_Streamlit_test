...
def predict_jruiz_week():
    df = pd.read_csv('static/j.ruiz.csv', parse_dates=['date'])
    time_slots = [
    ...
    if len(day_data) >= 4:
        ...
        plt.title(f'J. Ruiz Station Predicted Entries & Exits per Hour ({today_short})')
        plt.legend()
        plt.tight_layout()
        plt.savefig('static/images/jruiz_predicted_bar.png')
        plt.close()
