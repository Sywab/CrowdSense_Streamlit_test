...
def predict_santolan_week():
    df = pd.read_csv('static/santolan.csv', parse_dates=['date'])
    time_slots = [
    ...
    if len(day_data) >= 4:
        ...
        plt.title(f'Santolan Station Predicted Entries & Exits per Hour ({today_short})')
        plt.legend()
        plt.tight_layout()
        plt.savefig('static/images/santolan_predicted_bar.png')
        plt.close()
