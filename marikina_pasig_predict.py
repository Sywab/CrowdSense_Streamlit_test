...
def predict_marikina_pasig_week():
    df = pd.read_csv('static/marikina.pasig.csv', parse_dates=['date'])
    time_slots = [
    ...
    if len(day_data) >= 4:
        ...
        plt.title(f'Marikina - Pasig Station Predicted Entries & Exits per Hour ({today_short})')
        plt.legend()
        plt.tight_layout()
        plt.savefig('static/images/marikina_pasig_predicted_bar.png')
        plt.close()
