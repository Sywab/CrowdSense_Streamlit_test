...
def predict_antipolo_week():
    df = pd.read_csv('static/antipolo.csv', parse_dates=['date'])
    time_slots = [
    ...
    if len(day_data) >= 4:
        ...
        plt.title(f'Antipolo Station Predicted Entries & Exits per Hour ({today_short})')
        plt.legend()
        plt.tight_layout()
        plt.savefig('static/images/antipolo_predicted_bar.png')
        plt.close()
