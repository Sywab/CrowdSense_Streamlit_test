...
def predict_katipunan_week():
    df = pd.read_csv('static/katipunan.csv', parse_dates=['date'])
    time_slots = [
    ...
    if len(day_data) >= 4:
        ...
        plt.title(f'Katipunan Station Predicted Entries & Exits per Hour ({today_short})')
        plt.legend()
        plt.tight_layout()
        plt.savefig('static/images/katipunan_predicted_bar.png')
        plt.close()
