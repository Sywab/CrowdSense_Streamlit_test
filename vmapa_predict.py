...
def predict_vmapa_week():
    df = pd.read_csv('static/v.mapa.csv', parse_dates=['date'])
    time_slots = [
    ...
    if len(day_data) >= 4:
        ...
        plt.title(f'V. Mapa Station Predicted Entries & Exits per Hour ({today_short})')
        plt.legend()
        plt.tight_layout()
        plt.savefig('static/images/vmapa_predicted_bar.png')
        plt.close()
