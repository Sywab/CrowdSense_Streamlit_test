...
def predict_legarda_week():
    df = pd.read_csv('static/legarda.csv', parse_dates=['date'])
    time_slots = [
    ...
    if len(day_data) >= 4:
        ...
        plt.title(f'Legarda Station Predicted Entries & Exits per Hour ({today_short})')
        plt.legend()
        plt.tight_layout()
        plt.savefig('static/images/legarda_predicted_bar.png')
        plt.close()
