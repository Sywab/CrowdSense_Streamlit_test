...
def predict_anonas_week():
    df = pd.read_csv('static/anonas.csv', parse_dates=['date'])
    time_slots = [
    ...
    if len(day_data) >= 4:
        ...
        plt.title(f'Anonas Station Predicted Entries & Exits per Hour ({today_short})')
        plt.legend()
        plt.tight_layout()
        plt.savefig('static/images/anonas_predicted_bar.png')
        plt.close()
